...
pub struct SQ {
    ...
    pub async fn create_window(&mut self, title: &str, width: u32, height: u32) -> Result<u32> {
        let window = SQWindow::new(self.render_context.clone(), self.assets.clone(), title, width, height)?;
...
pub struct SQWindow {
    ...
    pub fn new(render_context: Arc<RenderContext>, assets: Arc<AssetManager>, title: &str, width: u32, height: u32) -> Result<Self> {
    ...
    pub fn resize(&mut self, device: &Device, width: u32, height: u32) {
        if width > 0 && height > 0 {
            ...
            self.size = Vector2 { x: width as f32, y: height as f32 };
